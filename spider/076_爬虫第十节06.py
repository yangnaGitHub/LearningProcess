# -*- coding: utf-8 -*-
"""
Created on Thu Jun 01 11:43:43 2017
...
def extract_user(users):
    print "extract user"
    for i in range(0, 20):
        for user_element in user_crawler.find_element_by_xpath('//*[contains(@class, "follow_item")]'):
            tried = 0
            while tried < 3:
                try:
                    user = {}
                    user['follow'] = re.findall('(\d+)', user_element.find_element_by_xpath('.//div[@class="info_connect"]/span').text)[0]
...
def scroll_to_bottom():
    ...
    for i in range(0,50):
        ...
        feeds_crawler.execute_script('window.scrollTo(0, document.body.scrollHeight)')#滑鼠
...
def fetch_user(user_link):
    ...
    # 提取用户姓名
    account_name = get_element_by_xpath(feeds_crawler, '//h1')[0].text#获取文本

    photo = get_element_by_xpath(feeds_crawler, '//p[@class="photo_wrap"]/img')[0].get('src')#获取属性

    account_photo = re.findall('/([^/]+)$', photo)#'/'至少一个/
    #'([^/]+)'以/开头的一个或多个字符
