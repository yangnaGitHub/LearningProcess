# -*- coding: utf-8 -*-
"""
Created on Wed Jun  6 15:51:42 2018
...
#构建模型 ==> 使用多层网络(使用3层网络)第一层1000,第二层1500,第三层输出10
#添加dropout层
l1 = 1000
l2 = 1500
...
#如何检验
result = tf.equal(tf.argmax(d3, 1), tf.argmax(y, 1))#1代表以列来取最大值下标
...
with tf.Session() as sess:
    ...
    drop_out_constant = 0.7
    for epoch in range(21):#跑多少圈
        #batch 训练
        for batchindex in range(each_batch):
            startindex = batchindex * batch_num
            sess.run(train, feed_dict={x:x_train[startindex:startindex+batch_num], y:y_train[startindex:startindex+batch_num], drop_out:drop_out_constant})
        acc = sess.run(accuracy, feed_dict={x:x_test, y:y_test, drop_out:drop_out_constant})
